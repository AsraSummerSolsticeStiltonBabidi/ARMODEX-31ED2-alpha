...
function resolveChordObject(chord, obj, ctx) {
  ...
  switch (obj.objectType) {
    ...
    // NOTE: common-tones is no longer a point-collision trigger (revision
    // round 2, per user feedback that direct touch was too rare to feel
    // alive) -- it's now a continuous AREA effect driven from the ecosystem
    // loop (see updateCommonTonesArea() in ecosystem.html / forces.js's
    // chordsWithinRadius + sharedPitchClassesAcross), so touching it directly
    // now just bounces physically like any other object (falls to default).
    case 'launcher': {
